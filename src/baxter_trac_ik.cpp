...
void findIKSolution(const KDL::Frame &end_effector_pose, const std::string &limbName)
{
  ...
  nominal(0) = joint_state[0];
  nominal(1) = joint_state[1];
  nominal(2) = joint_state[2];
  nominal(3) = joint_state[3];
  nominal(4) = joint_state[4];
  nominal(5) = joint_state[5];
  nominal(6) = joint_state[6];
...
void callBack_marker_change(const geometry_msgs::Pose callBackData){	
  ...
  cout << callBackData;
  tf::poseMsgToKDL(callBackData, end_effector_pose);
    // Compute the IK Solution and publish the solution from within the method
  findIKSolution(end_effector_pose, "right");
...
void callBack2(const sensor_msgs::JointState &callBackData1){
...
int main(int argc, char** argv)
{
  ...
  ros::Subscriber subscriber = nodeHandle.subscribe("/aruco_marker_change", 1, callBack_marker_change);
  
  
  ros::Subscriber subscriber1 = nodeHandle.subscribe("/robot/joint_states", 1, callBack2);
